...
createTC <- function(categories = NULL,
  ...
  # Load category data  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  suppressMessages(
    catags <- readr::read_csv(categories)
  )
...
# Create a seperate sick and leave time card - add to main one later ~~~~~~~~~~~
  ...
  if (tools::file_ext(outCal) == 'xlsx') {
    ...
    calTidy <- calendar %>%
      ...
      filter(StartDT >= week_start & StartDT <= as.Date(week_start) +6) %>%
      mutate(Length = as.numeric(
        difftime(EndDT, StartDT, units ='secs')
        /3600))

    # Fix dates of all day events ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # Midnight start forces into another day ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    calTidy <- calTidy %>%
      filter(Categories != 'Ignore & Leave' &
               Categories != 'Duty' & Categories != 'Holiday' &
               !is.na(Categories)) %>%
      select(Date, Subject = Event, Categories, StartDT, EndDT, Length, allDay)

  } else {
    stop('Please ensure an xlsx file generated by the power automate process is used. Raw calendar exports from outlook do not provide the correct time stamps for reacurrring appointments')
  }
